//These functions are used to control the OLED module which uses a SSD1306 controller.
...
#define SET_MULTIPLEX_RATIO		0xA8	//Following byte sets the multiplex mode (16to63).  This depends on the display's physical resolution.
...
#define PAGE_ADDRESS_MODE		0x02	//Following the SET_MEMORY_ADDRESSING_MODE, this will put the module into page addressing mode (default).
#define SET_SEGMENT_REMAP_0		0xA0	//Segment re-map defines which column address is mapped to SEG0 (first data byte).  This is the default.
...
#define SET_LOW_COLUMN_START_ADDRESS	0x00	//Combine with the next command to give the full byte indicating the column start address.
						//"OR" 0x00 with 4 bits to give the lower nibble.
#define SET_HIGH_COLUMN_START_ADDRESS	0x10	//and "OR" 0x10 with 4 bits to give the upper nibble.
						//I.e. (0x00 | 0b0000EFGH),  (0x01 | 0b0000ABCD) gives column start address 0bABCDEFGH.
#define SET_PAGE_START_ADDRESS		0xB0	//This command by itself sets the page start address to page 0.
						//"OR" with 0x0 to 0x7 to set pages 0 to 7 (I.e. page range is 0xB0 to 0xB7).

//Notes on page addressing mode (for 64x128 display):
//Address Pointer Movement (note, no wrapping to next page).
...
//Each byte (segment) is effectively a vertical line of 8 pixels:
//i.e. 	Byte/		b0	<-LSb
//	Segment:	b1
//			b2
//			b3
