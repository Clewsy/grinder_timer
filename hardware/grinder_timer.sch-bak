EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Text Notes 7700 6950 0    79   ~ 0
refer to:\nhttps://clews.pro/projects/grinder_timer\nhttps://gitlab.com/clewsy/grinder_timer
Text Notes 10600 7650 0    79   ~ 0
2
Text Notes 7400 7500 0    79   ~ 0
grinder_timer_schematic
$Comp
L Device:Transformer_1P_1S T1
U 1 1 5F0022EB
P 1900 2750
F 0 "T1" H 1900 3000 50  0000 C CNN
F 1 "1P1S" H 1900 2500 50  0000 C CNN
F 2 "grinder_timer:transformer_big" H 1900 2750 50  0001 C CNN
F 3 "~" H 1900 2750 50  0001 C CNN
	1    1900 2750
	1    0    0    -1  
$EndComp
$Comp
L Device:D_Bridge_+-AA B1
U 1 1 5F009CE5
P 2800 2900
F 0 "B1" H 2850 3100 50  0000 L CNN
F 1 "D_Bridge" H 2850 2700 50  0000 L CNN
F 2 "Diode_SMD:Diode_Bridge_Vishay_DFS" H 2800 2900 50  0001 C CNN
F 3 "~" H 2800 2900 50  0001 C CNN
	1    2800 2900
	1    0    0    -1  
$EndComp
$Comp
L Device:C C2
U 1 1 5F00BA7C
P 3550 3100
F 0 "C2" H 3550 3200 50  0000 L CNN
F 1 "0.1uF" H 3550 3000 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric_Pad1.42x1.75mm_HandSolder" H 3588 2950 50  0001 C CNN
F 3 "~" H 3550 3100 50  0001 C CNN
	1    3550 3100
	1    0    0    -1  
$EndComp
$Comp
L Device:C C3
U 1 1 5F00BD5C
P 4250 3100
F 0 "C3" H 4250 3200 50  0000 L CNN
F 1 "0.1uF" H 4250 3000 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric_Pad1.42x1.75mm_HandSolder" H 4288 2950 50  0001 C CNN
F 3 "~" H 4250 3100 50  0001 C CNN
	1    4250 3100
	1    0    0    -1  
$EndComp
$Comp
L Device:CP C1
U 1 1 5F00C1D1
P 3300 3100
F 0 "C1" H 3300 3200 50  0000 L CNN
F 1 "470uF" H 3300 3000 50  0000 L CNN
F 2 "Capacitor_SMD:C_Elec_8x10.2" H 3338 2950 50  0001 C CNN
F 3 "~" H 3300 3100 50  0001 C CNN
	1    3300 3100
	1    0    0    -1  
$EndComp
$Comp
L Device:CP C4
U 1 1 5F00C5BF
P 4500 3100
F 0 "C4" H 4500 3200 50  0000 L CNN
F 1 "10uF" H 4500 3000 50  0000 L CNN
F 2 "Capacitor_SMD:C_Elec_5x5.4" H 4538 2950 50  0001 C CNN
F 3 "~" H 4500 3100 50  0001 C CNN
	1    4500 3100
	1    0    0    -1  
$EndComp
$Comp
L MCU_Microchip_ATmega:ATmega328PB-AU U1
U 1 1 5F00CADF
P 1300 5100
F 0 "U1" H 950 3650 50  0000 C CNN
F 1 "ATmega328PB-AU" H 1200 5100 50  0000 C CNN
F 2 "Package_QFP:TQFP-32_7x7mm_P0.8mm" H 1300 5100 50  0001 C CIN
F 3 "http://ww1.microchip.com/downloads/en/DeviceDoc/40001906C.pdf" H 1300 5100 50  0001 C CNN
	1    1300 5100
	1    0    0    -1  
$EndComp
$Comp
L Connector:AVR-ISP-6 J1
U 1 1 5F0142D8
P 4050 6750
F 0 "J1" H 3721 6846 50  0000 R CNN
F 1 "AVR-ISP-6" H 3721 6755 50  0000 R CNN
F 2 "Connector_IDC:IDC-Header_2x03_P2.54mm_Vertical" V 3800 6800 50  0001 C CNN
F 3 " ~" H 2775 6200 50  0001 C CNN
	1    4050 6750
	1    0    0    -1  
$EndComp
$Comp
L Switch:SW_Push S0
U 1 1 5F0151C0
P 4000 5550
F 0 "S0" H 4100 5650 50  0000 C CNN
F 1 "RESET" H 4000 5500 50  0000 C CNN
F 2 "Button_Switch_SMD:SW_Push_1P1T_NO_6x6mm_H9.5mm" H 4000 5750 50  0001 C CNN
F 3 "~" H 4000 5750 50  0001 C CNN
	1    4000 5550
	1    0    0    -1  
$EndComp
$Comp
L Switch:SW_Push S1
U 1 1 5F0159D7
P 2350 5950
F 0 "S1" H 2450 6050 50  0000 C CNN
F 1 "UP" H 2350 5900 50  0000 C CNN
F 2 "grinder_timer:tact" H 2350 6150 50  0001 C CNN
F 3 "~" H 2350 6150 50  0001 C CNN
	1    2350 5950
	1    0    0    -1  
$EndComp
$Comp
L Switch:SW_Push S2
U 1 1 5F015D1F
P 2350 6200
F 0 "S2" H 2450 6300 50  0000 C CNN
F 1 "DOWN" H 2350 6150 50  0000 C CNN
F 2 "grinder_timer:tact" H 2350 6400 50  0001 C CNN
F 3 "~" H 2350 6400 50  0001 C CNN
	1    2350 6200
	1    0    0    -1  
$EndComp
$Comp
L Device:R R2
U 1 1 5F016EBC
P 3850 5000
F 0 "R2" H 3900 5050 50  0000 L CNN
F 1 "3.3k" H 3900 4950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric_Pad1.42x1.75mm_HandSolder" V 3780 5000 50  0001 C CNN
F 3 "~" H 3850 5000 50  0001 C CNN
	1    3850 5000
	1    0    0    -1  
$EndComp
$Comp
L Device:R R1
U 1 1 5F01731E
P 3450 5000
F 0 "R1" H 3500 5050 50  0000 L CNN
F 1 "3.3k" H 3500 4950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric_Pad1.42x1.75mm_HandSolder" V 3380 5000 50  0001 C CNN
F 3 "~" H 3450 5000 50  0001 C CNN
	1    3450 5000
	1    0    0    -1  
$EndComp
$Comp
L Device:R R3
U 1 1 5F017606
P 4250 5000
F 0 "R3" H 4300 5050 50  0000 L CNN
F 1 "10k" H 4300 4950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric_Pad1.42x1.75mm_HandSolder" V 4180 5000 50  0001 C CNN
F 3 "~" H 4250 5000 50  0001 C CNN
	1    4250 5000
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0101
U 1 1 5F03D2A7
P 2600 2250
F 0 "#PWR0101" H 2600 2000 50  0001 C CNN
F 1 "GND" H 2600 2100 50  0000 C CNN
F 2 "" H 2600 2250 50  0001 C CNN
F 3 "" H 2600 2250 50  0001 C CNN
	1    2600 2250
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0102
U 1 1 5F0650CF
P 2450 2950
F 0 "#PWR0102" H 2450 2700 50  0001 C CNN
F 1 "GND" H 2450 2800 50  0000 C CNN
F 2 "" H 2450 2950 50  0001 C CNN
F 3 "" H 2450 2950 50  0001 C CNN
	1    2450 2950
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0103
U 1 1 5F084609
P 4550 3350
F 0 "#PWR0103" H 4550 3100 50  0001 C CNN
F 1 "GND" H 4550 3200 50  0000 C CNN
F 2 "" H 4550 3350 50  0001 C CNN
F 3 "" H 4550 3350 50  0001 C CNN
	1    4550 3350
	1    0    0    -1  
$EndComp
Text GLabel 1900 2000 0    79   Input ~ 0
RELAY
Wire Wire Line
	1500 2950 1450 2950
Wire Wire Line
	1400 2850 1450 2850
Wire Wire Line
	1450 2850 1450 2950
Connection ~ 1450 2950
Wire Wire Line
	1450 2950 1400 2950
Wire Wire Line
	1400 2550 1450 2550
Wire Wire Line
	1400 2650 1450 2650
Wire Wire Line
	1450 2650 1450 2550
Connection ~ 1450 2550
Wire Wire Line
	1450 2550 1500 2550
Wire Wire Line
	2500 2900 2450 2900
Wire Wire Line
	2450 2900 2450 2950
Wire Wire Line
	2300 2950 2350 2950
Wire Wire Line
	2350 2950 2350 3250
Wire Wire Line
	2350 3250 2800 3250
Wire Wire Line
	2800 3250 2800 3200
Wire Wire Line
	2800 2550 2800 2600
Wire Wire Line
	3100 2900 3150 2900
Wire Wire Line
	3550 2950 3550 2900
Connection ~ 3550 2900
Wire Wire Line
	3550 2900 3600 2900
Wire Wire Line
	3300 2950 3300 2900
Connection ~ 3300 2900
Wire Wire Line
	3300 2900 3550 2900
Wire Wire Line
	3300 3250 3300 3300
Wire Wire Line
	3300 3300 3550 3300
Wire Wire Line
	3900 3200 3900 3300
Wire Wire Line
	3550 3300 3550 3250
Connection ~ 3550 3300
Wire Wire Line
	3550 3300 3900 3300
Wire Wire Line
	4500 2900 4500 2950
Wire Wire Line
	4200 2900 4250 2900
Wire Wire Line
	4550 2900 4550 2850
Connection ~ 4500 2900
Wire Wire Line
	4500 2900 4550 2900
Wire Wire Line
	4550 3300 4550 3350
Wire Wire Line
	4500 3300 4550 3300
Wire Wire Line
	4500 3250 4500 3300
Wire Wire Line
	4250 3300 4250 3250
Connection ~ 3900 3300
Wire Wire Line
	3900 3300 4250 3300
Connection ~ 4500 3300
Connection ~ 4250 3300
Wire Wire Line
	4250 3300 4500 3300
Wire Wire Line
	4250 2950 4250 2900
Connection ~ 4250 2900
Wire Wire Line
	4250 2900 4500 2900
Text GLabel 4550 6400 2    79   Input ~ 0
MISO
Text GLabel 4550 6600 2    79   Input ~ 0
MOSI
Text GLabel 4550 6800 2    79   Input ~ 0
SCK
Text GLabel 4550 7000 2    79   Input ~ 0
!RST
$Comp
L power:GND #PWR0104
U 1 1 5F0D8192
P 3950 7200
F 0 "#PWR0104" H 3950 6950 50  0001 C CNN
F 1 "GND" H 3955 7027 50  0000 C CNN
F 2 "" H 3950 7200 50  0001 C CNN
F 3 "" H 3950 7200 50  0001 C CNN
	1    3950 7200
	1    0    0    -1  
$EndComp
Wire Wire Line
	4450 6850 4500 6850
Wire Wire Line
	4500 6850 4500 7000
Wire Wire Line
	4500 7000 4550 7000
Wire Wire Line
	4450 6750 4500 6750
Wire Wire Line
	4500 6750 4500 6800
Wire Wire Line
	4500 6800 4550 6800
Wire Wire Line
	4450 6650 4500 6650
Wire Wire Line
	4500 6650 4500 6600
Wire Wire Line
	4500 6600 4550 6600
Wire Wire Line
	4450 6550 4500 6550
Wire Wire Line
	4500 6550 4500 6400
Wire Wire Line
	4500 6400 4550 6400
Wire Wire Line
	3950 6200 3950 6250
Wire Wire Line
	3950 7150 3950 7200
Wire Wire Line
	1400 3500 1400 3550
Wire Wire Line
	1300 3600 1300 3550
Wire Wire Line
	1300 3550 1400 3550
Connection ~ 1400 3550
Wire Wire Line
	1400 3550 1400 3600
$Comp
L power:GND #PWR0105
U 1 1 5F11D0E6
P 1300 6650
F 0 "#PWR0105" H 1300 6400 50  0001 C CNN
F 1 "GND" H 1300 6500 50  0000 C CNN
F 2 "" H 1300 6650 50  0001 C CNN
F 3 "" H 1300 6650 50  0001 C CNN
	1    1300 6650
	1    0    0    -1  
$EndComp
Wire Wire Line
	1300 6650 1300 6600
NoConn ~ 700  3900
Text GLabel 4300 5400 2    79   Input ~ 0
!RST
Text GLabel 2000 4400 2    79   Input ~ 0
SCK
Text GLabel 2000 4250 2    79   Input ~ 0
MISO
Text GLabel 2000 4100 2    79   Input ~ 0
MOSI
Wire Wire Line
	1900 4400 2000 4400
Wire Wire Line
	1900 4300 1950 4300
Wire Wire Line
	1950 4300 1950 4250
Wire Wire Line
	1950 4250 2000 4250
Wire Wire Line
	1900 4200 1950 4200
Wire Wire Line
	1950 4200 1950 4100
Wire Wire Line
	1950 4100 2000 4100
NoConn ~ 1900 4100
NoConn ~ 1900 4000
NoConn ~ 1900 3900
Text GLabel 2000 4800 2    79   Input ~ 0
RELAY
Wire Wire Line
	2550 4700 2550 4750
Wire Wire Line
	2600 4700 2550 4700
Wire Wire Line
	3150 4500 3250 4500
$Comp
L power:GND #PWR0106
U 1 1 5F0EF57B
P 3250 5000
F 0 "#PWR0106" H 3250 4750 50  0001 C CNN
F 1 "GND" H 3250 4850 50  0000 C CNN
F 2 "" H 3250 5000 50  0001 C CNN
F 3 "" H 3250 5000 50  0001 C CNN
	1    3250 5000
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0107
U 1 1 5F0EA094
P 2550 4750
F 0 "#PWR0107" H 2550 4500 50  0001 C CNN
F 1 "GND" H 2550 4600 50  0000 C CNN
F 2 "" H 2550 4750 50  0001 C CNN
F 3 "" H 2550 4750 50  0001 C CNN
	1    2550 4750
	1    0    0    -1  
$EndComp
$Comp
L Device:C C6
U 1 1 5F013BB4
P 3000 4950
F 0 "C6" V 3050 4850 50  0000 C CNN
F 1 "17pF" V 3050 5100 50  0000 C CNN
F 2 "Capacitor_SMD:C_1206_3216Metric_Pad1.42x1.75mm_HandSolder" H 3038 4800 50  0001 C CNN
F 3 "~" H 3000 4950 50  0001 C CNN
	1    3000 4950
	0    1    1    0   
$EndComp
$Comp
L Device:C C5
U 1 1 5F0137FD
P 3000 4500
F 0 "C5" V 2950 4400 50  0000 C CNN
F 1 "7pF" V 2950 4600 50  0000 C CNN
F 2 "Capacitor_SMD:C_1206_3216Metric_Pad1.42x1.75mm_HandSolder" H 3038 4350 50  0001 C CNN
F 3 "~" H 3000 4500 50  0001 C CNN
	1    3000 4500
	0    1    1    0   
$EndComp
$Comp
L Device:Crystal_GND23 Y1
U 1 1 5F012F2F
P 2800 4700
F 0 "Y1" V 2650 4500 50  0000 L CNN
F 1 "32.768kHz" V 2800 4600 25  0000 L CNN
F 2 "Crystal:Crystal_SMD_Abracon_ABS25-4Pin_8.0x3.8mm" H 2800 4700 50  0001 C CNN
F 3 "~" H 2800 4700 50  0001 C CNN
	1    2800 4700
	0    1    1    0   
$EndComp
Wire Wire Line
	2850 4500 2800 4500
Wire Wire Line
	1900 4600 2450 4600
Wire Wire Line
	1900 4800 2000 4800
Wire Wire Line
	2450 4600 2450 4950
Wire Wire Line
	2450 4950 2800 4950
Wire Wire Line
	2800 4950 2800 4850
Wire Wire Line
	2800 4950 2850 4950
Connection ~ 2800 4950
Wire Wire Line
	3150 4950 3250 4950
Wire Wire Line
	3250 4950 3250 5000
Wire Wire Line
	3250 4500 3250 4700
Connection ~ 3250 4950
Wire Wire Line
	3000 4700 3250 4700
Connection ~ 3250 4700
Wire Wire Line
	3250 4700 3250 4950
Wire Wire Line
	2800 4550 2800 4500
Connection ~ 2800 4500
Wire Wire Line
	2800 4500 1900 4500
Text GLabel 3500 5200 2    79   Input ~ 0
SDA
Text GLabel 3900 5300 2    79   Input ~ 0
SCL
Wire Wire Line
	3500 5200 3450 5200
Wire Wire Line
	3450 5150 3450 5200
Connection ~ 3450 5200
Wire Wire Line
	3450 5200 1900 5200
Wire Wire Line
	4300 5400 4250 5400
Wire Wire Line
	1900 5300 3850 5300
Wire Wire Line
	4250 4750 4250 4800
Wire Wire Line
	3450 4850 3450 4800
Wire Wire Line
	3450 4800 3850 4800
Connection ~ 4250 4800
Wire Wire Line
	4250 4800 4250 4850
Wire Wire Line
	3850 4800 3850 4850
Connection ~ 3850 4800
Wire Wire Line
	3850 4800 4250 4800
Wire Wire Line
	3850 5150 3850 5300
Connection ~ 3850 5300
Wire Wire Line
	3850 5300 3900 5300
Wire Wire Line
	4250 5150 4250 5400
Connection ~ 4250 5400
Wire Wire Line
	4250 5400 1900 5400
$Comp
L power:GND #PWR0108
U 1 1 5F1E147E
P 3750 5600
F 0 "#PWR0108" H 3750 5350 50  0001 C CNN
F 1 "GND" H 3750 5450 50  0000 C CNN
F 2 "" H 3750 5600 50  0001 C CNN
F 3 "" H 3750 5600 50  0001 C CNN
	1    3750 5600
	1    0    0    -1  
$EndComp
Text GLabel 2000 5650 2    79   Input ~ 0
AVR_TX
Text GLabel 2000 5500 2    79   Input ~ 0
AVR_RX
$Comp
L Switch:SW_Push S4
U 1 1 5F0163F9
P 2350 6700
F 0 "S4" H 2450 6800 50  0000 C CNN
F 1 "RIGHT" H 2350 6650 50  0000 C CNN
F 2 "grinder_timer:tact" H 2350 6900 50  0001 C CNN
F 3 "~" H 2350 6900 50  0001 C CNN
	1    2350 6700
	1    0    0    -1  
$EndComp
$Comp
L Switch:SW_Push S3
U 1 1 5F016052
P 2350 6450
F 0 "S3" H 2450 6550 50  0000 C CNN
F 1 "LEFT" H 2350 6400 50  0000 C CNN
F 2 "grinder_timer:tact" H 2350 6650 50  0001 C CNN
F 3 "~" H 2350 6650 50  0001 C CNN
	1    2350 6450
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0109
U 1 1 5F21B4C7
P 2700 6750
F 0 "#PWR0109" H 2700 6500 50  0001 C CNN
F 1 "GND" H 2700 6600 50  0000 C CNN
F 2 "" H 2700 6750 50  0001 C CNN
F 3 "" H 2700 6750 50  0001 C CNN
	1    2700 6750
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x06 J2
U 1 1 5F23AF4C
P 5650 6800
F 0 "J2" H 5600 7100 50  0000 L CNN
F 1 "SERIAL" H 5550 6400 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x06_P2.54mm_Vertical" H 5650 6800 50  0001 C CNN
F 3 "~" H 5650 6800 50  0001 C CNN
	1    5650 6800
	-1   0    0    1   
$EndComp
Text GLabel 5750 5750 0    79   Input ~ 0
SDA
Text GLabel 5750 5600 0    79   Input ~ 0
SCL
$Comp
L grinder_timer:OLED O1
U 1 1 5F241E8E
P 6050 5300
F 0 "O1" V 5800 5250 79  0000 L CNN
F 1 "OLED" V 6300 5250 79  0000 L CNN
F 2 "grinder_timer:oled" H 6050 5550 79  0001 C CNN
F 3 "" H 6050 5550 79  0001 C CNN
	1    6050 5300
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0110
U 1 1 5F2536F4
P 5700 5300
F 0 "#PWR0110" H 5700 5050 50  0001 C CNN
F 1 "GND" H 5700 5150 50  0000 C CNN
F 2 "" H 5700 5300 50  0001 C CNN
F 3 "" H 5700 5300 50  0001 C CNN
	1    5700 5300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0111
U 1 1 5F27CBD9
P 6050 7050
F 0 "#PWR0111" H 6050 6800 50  0001 C CNN
F 1 "GND" H 6050 6900 50  0000 C CNN
F 2 "" H 6050 7050 50  0001 C CNN
F 3 "" H 6050 7050 50  0001 C CNN
	1    6050 7050
	1    0    0    -1  
$EndComp
Text Notes 5350 7050 0    67   ~ 0
!DTR\nRX\nTX\nVCC\n!CTS\nGND
Text GLabel 6000 6450 2    79   Input ~ 0
AVR_RX
Text GLabel 6000 6300 2    79   Input ~ 0
AVR_TX
NoConn ~ 5850 6500
NoConn ~ 5850 6900
NoConn ~ 700  5900
NoConn ~ 700  5800
NoConn ~ 700  5700
NoConn ~ 700  5600
NoConn ~ 1900 5100
NoConn ~ 1900 5000
NoConn ~ 1900 4900
Wire Wire Line
	4200 5550 4250 5550
Wire Wire Line
	4250 5550 4250 5400
Wire Wire Line
	3800 5550 3750 5550
Wire Wire Line
	3750 5550 3750 5600
Wire Wire Line
	2000 5650 1950 5650
Wire Wire Line
	1950 5650 1950 5700
Wire Wire Line
	1950 5700 1900 5700
Wire Wire Line
	2000 5500 1950 5500
Wire Wire Line
	1950 5500 1950 5600
Wire Wire Line
	1950 5600 1900 5600
$Comp
L Device:Q_NPN_BCE Q1
U 1 1 5F349A8F
P 2500 2000
F 0 "Q1" H 2691 2046 50  0000 L CNN
F 1 "NPN" H 2691 1955 50  0000 L CNN
F 2 "Package_TO_SOT_SMD:SOT-223-3_TabPin2" H 2700 2100 50  0001 C CNN
F 3 "~" H 2500 2000 50  0001 C CNN
	1    2500 2000
	1    0    0    -1  
$EndComp
$Comp
L Device:D D1
U 1 1 5F34ACA3
P 2250 1400
F 0 "D1" V 2150 1400 50  0000 L CNN
F 1 "4148" V 2350 1200 50  0000 L CNN
F 2 "Diode_SMD:D_SOD-123" H 2250 1400 50  0001 C CNN
F 3 "~" H 2250 1400 50  0001 C CNN
	1    2250 1400
	0    1    1    0   
$EndComp
$Comp
L Device:R R4
U 1 1 5F380CA6
P 2100 7250
F 0 "R4" H 2150 7300 50  0000 L CNN
F 1 "150" H 2150 7200 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric_Pad1.42x1.75mm_HandSolder" V 2030 7250 50  0001 C CNN
F 3 "~" H 2100 7250 50  0001 C CNN
	1    2100 7250
	-1   0    0    1   
$EndComp
Wire Wire Line
	1900 5900 2100 5900
Wire Wire Line
	2100 5900 2100 5950
Wire Wire Line
	2100 5950 2150 5950
Wire Wire Line
	1900 6000 2100 6000
Wire Wire Line
	2100 6000 2100 6200
Wire Wire Line
	2100 6200 2150 6200
Wire Wire Line
	1900 6100 2050 6100
Wire Wire Line
	2050 6100 2050 6450
Wire Wire Line
	2050 6450 2150 6450
Wire Wire Line
	1900 6200 2000 6200
Wire Wire Line
	2000 6200 2000 6700
Wire Wire Line
	2000 6700 2150 6700
Wire Wire Line
	1900 6300 1950 6300
Wire Wire Line
	1950 6300 1950 6950
Wire Wire Line
	1950 6950 2150 6950
Wire Wire Line
	2550 6950 2600 6950
Wire Wire Line
	2600 6950 2600 6700
Wire Wire Line
	2600 5950 2550 5950
Wire Wire Line
	2550 6200 2600 6200
Connection ~ 2600 6200
Wire Wire Line
	2600 6200 2600 5950
Wire Wire Line
	2550 6450 2600 6450
Connection ~ 2600 6450
Wire Wire Line
	2600 6450 2600 6200
Wire Wire Line
	2550 6700 2600 6700
Connection ~ 2600 6700
Wire Wire Line
	2600 6700 2600 6450
Wire Wire Line
	2600 6700 2700 6700
Wire Wire Line
	2700 6700 2700 6750
Wire Wire Line
	1900 5800 2800 5800
Wire Wire Line
	2800 5800 2800 7050
Wire Wire Line
	2800 7050 2550 7050
Wire Wire Line
	2150 7050 2100 7050
Wire Wire Line
	2100 7050 2100 7100
$Comp
L power:GND #PWR0112
U 1 1 5F452FC8
P 2100 7450
F 0 "#PWR0112" H 2100 7200 50  0001 C CNN
F 1 "GND" H 2100 7300 50  0000 C CNN
F 2 "" H 2100 7450 50  0001 C CNN
F 3 "" H 2100 7450 50  0001 C CNN
	1    2100 7450
	1    0    0    -1  
$EndComp
Wire Wire Line
	2100 7400 2100 7450
$Comp
L power:+5V #PWR0113
U 1 1 5F46398A
P 4550 2850
F 0 "#PWR0113" H 4550 2700 50  0001 C CNN
F 1 "+5V" H 4550 3000 50  0000 C CNN
F 2 "" H 4550 2850 50  0001 C CNN
F 3 "" H 4550 2850 50  0001 C CNN
	1    4550 2850
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0114
U 1 1 5F46494D
P 3950 6200
F 0 "#PWR0114" H 3950 6050 50  0001 C CNN
F 1 "+5V" H 3950 6350 50  0000 C CNN
F 2 "" H 3950 6200 50  0001 C CNN
F 3 "" H 3950 6200 50  0001 C CNN
	1    3950 6200
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0115
U 1 1 5F4659A4
P 5700 5100
F 0 "#PWR0115" H 5700 4950 50  0001 C CNN
F 1 "+5V" H 5700 5250 50  0000 C CNN
F 2 "" H 5700 5100 50  0001 C CNN
F 3 "" H 5700 5100 50  0001 C CNN
	1    5700 5100
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0116
U 1 1 5F4660BA
P 6050 6750
F 0 "#PWR0116" H 6050 6600 50  0001 C CNN
F 1 "+5V" H 6050 6900 50  0000 C CNN
F 2 "" H 6050 6750 50  0001 C CNN
F 3 "" H 6050 6750 50  0001 C CNN
	1    6050 6750
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0117
U 1 1 5F4672EA
P 4250 4750
F 0 "#PWR0117" H 4250 4600 50  0001 C CNN
F 1 "+5V" H 4250 4900 50  0000 C CNN
F 2 "" H 4250 4750 50  0001 C CNN
F 3 "" H 4250 4750 50  0001 C CNN
	1    4250 4750
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0118
U 1 1 5F4699EA
P 1400 3500
F 0 "#PWR0118" H 1400 3350 50  0001 C CNN
F 1 "+5V" H 1400 3650 50  0000 C CNN
F 2 "" H 1400 3500 50  0001 C CNN
F 3 "" H 1400 3500 50  0001 C CNN
	1    1400 3500
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0119
U 1 1 5F4799E7
P 2600 1000
F 0 "#PWR0119" H 2600 850 50  0001 C CNN
F 1 "+5V" H 2600 1150 50  0000 C CNN
F 2 "" H 2600 1000 50  0001 C CNN
F 3 "" H 2600 1000 50  0001 C CNN
	1    2600 1000
	1    0    0    -1  
$EndComp
$Comp
L grinder_timer:terminal P1
U 1 1 5F001DA1
P 1050 2300
F 0 "P1" H 1050 2350 79  0000 C CNN
F 1 "MOTOR" H 1050 2250 79  0000 C CNN
F 2 "grinder_timer:pcb_tab" H 1050 2300 79  0001 C CNN
F 3 "" H 1050 2300 79  0001 C CNN
	1    1050 2300
	1    0    0    -1  
$EndComp
$Comp
L grinder_timer:terminal P3
U 1 1 5F001AFE
P 1050 2900
F 0 "P3" H 1050 2950 79  0000 C CNN
F 1 "ACTIVE" H 1050 2850 79  0000 C CNN
F 2 "grinder_timer:pcb_tab" H 1050 2900 79  0001 C CNN
F 3 "" H 1050 2900 79  0001 C CNN
	1    1050 2900
	1    0    0    -1  
$EndComp
$Comp
L grinder_timer:terminal P2
U 1 1 5F001657
P 1050 2600
F 0 "P2" H 1050 2650 79  0000 C CNN
F 1 "NEUTRAL" H 1050 2550 79  0000 C CNN
F 2 "grinder_timer:pcb_tab" H 1050 2600 79  0001 C CNN
F 3 "" H 1050 2600 79  0001 C CNN
	1    1050 2600
	1    0    0    -1  
$EndComp
Wire Wire Line
	1450 2350 1400 2350
Wire Wire Line
	1400 2250 1450 2250
Wire Wire Line
	1450 2250 1450 2350
$Comp
L Regulator_Linear:AP1117-50 U2
U 1 1 5F5A5924
P 3900 2900
F 0 "U2" H 3900 3142 50  0000 C CNN
F 1 "5V" H 3900 3051 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-223-3_TabPin2" H 3900 3100 50  0001 C CNN
F 3 "http://www.diodes.com/datasheets/AP1117.pdf" H 4000 2650 50  0001 C CNN
	1    3900 2900
	1    0    0    -1  
$EndComp
NoConn ~ 2900 1100
Wire Wire Line
	5850 7000 6050 7000
Wire Wire Line
	6050 7000 6050 7050
Wire Wire Line
	6050 6750 6050 6800
Wire Wire Line
	6050 6800 5850 6800
Wire Wire Line
	5850 6700 5950 6700
Wire Wire Line
	5950 6700 5950 6450
Wire Wire Line
	5950 6450 6000 6450
Wire Wire Line
	5850 6600 5900 6600
Wire Wire Line
	5900 6600 5900 6300
Wire Wire Line
	5900 6300 6000 6300
Wire Wire Line
	5900 5450 5850 5450
Wire Wire Line
	5850 5450 5850 5750
Wire Wire Line
	5850 5750 5750 5750
Wire Wire Line
	5750 5600 5800 5600
Wire Wire Line
	5800 5600 5800 5350
Wire Wire Line
	5800 5350 5900 5350
Wire Wire Line
	5900 5250 5700 5250
Wire Wire Line
	5700 5250 5700 5300
Wire Wire Line
	5700 5100 5700 5150
Wire Wire Line
	5700 5150 5900 5150
$Comp
L power:PWR_FLAG #FLG0102
U 1 1 5F74446E
P 3150 2900
F 0 "#FLG0102" H 3150 2975 50  0001 C CNN
F 1 "PWR_FLAG" H 3150 3050 50  0000 C CNN
F 2 "" H 3150 2900 50  0001 C CNN
F 3 "~" H 3150 2900 50  0001 C CNN
	1    3150 2900
	1    0    0    -1  
$EndComp
$Comp
L power:PWR_FLAG #FLG0101
U 1 1 5F743C01
P 2450 2900
F 0 "#FLG0101" H 2450 2975 50  0001 C CNN
F 1 "PWR_FLAG" H 2450 3050 50  0000 C CNN
F 2 "" H 2450 2900 50  0001 C CNN
F 3 "~" H 2450 2900 50  0001 C CNN
	1    2450 2900
	1    0    0    -1  
$EndComp
Wire Wire Line
	2300 2550 2800 2550
Connection ~ 2450 2900
Connection ~ 3150 2900
Wire Wire Line
	3150 2900 3300 2900
$Comp
L Device:R R5
U 1 1 5F765E44
P 2100 2000
F 0 "R5" H 2150 2050 50  0000 L CNN
F 1 "10k" H 2150 1950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric_Pad1.42x1.75mm_HandSolder" V 2030 2000 50  0001 C CNN
F 3 "~" H 2100 2000 50  0001 C CNN
	1    2100 2000
	0    1    1    0   
$EndComp
Wire Wire Line
	2600 1700 2600 1750
Wire Wire Line
	2250 1550 2250 1750
Wire Wire Line
	2250 1750 2600 1750
Connection ~ 2600 1750
Wire Wire Line
	2600 1750 2600 1800
Wire Wire Line
	2600 1000 2600 1050
Wire Wire Line
	2600 1050 2250 1050
Wire Wire Line
	2250 1050 2250 1250
Connection ~ 2600 1050
Wire Wire Line
	2600 1050 2600 1100
Wire Wire Line
	1900 2000 1950 2000
Wire Wire Line
	1450 2250 1450 800 
Wire Wire Line
	1450 800  3100 800 
Wire Wire Line
	3100 800  3100 1100
Connection ~ 1450 2250
Wire Wire Line
	2250 2000 2300 2000
Wire Wire Line
	1450 2550 1450 2450
Wire Wire Line
	1450 2450 3000 2450
Wire Wire Line
	3000 2450 3000 1700
Wire Wire Line
	2600 2250 2600 2200
Text Notes 7050 5100 0    79   ~ 0
BOM\n----------------------------------------\nB1 - DF04S - Bridge Rectifier\nC1 - EMVY160ADA471MHA0G - 470uF Electrolytic Capacitor\nC2 - 0.1uF 1206 Ceramic Capacitor\nC3 - 0.1uF 1206 Ceramic Capacitor\nC4 - MCESL35V106M5X5.2 - 10uF Electrolytic Capacitor\nC5 - 7pF 1206 Ceramic Capacitor\nC6 - 17pF 1206 Ceramic Capacitor\nD1 - T4 1N4148W SOD-123 Diode\nJ1 - 6-Pin (2x3) ISP Header\nJ2 - 6-Pin (1x6) Serial Header\nK1 - SRM-1C-SL-5VDC - Relay\nO1 - OLED Display w/ SH1106 Driver in I2C Mode\nP1 - 19708-4001 - PCB-Mount Spade Terminal\nP2 - 19708-4001 - PCB-Mount Spade Terminal\nP3 - 19708-4001 - PCB-Mount Spade Terminal\nQ1 - PBSS4350Z - NPN Transistor\nR1 - 1.8k Ohm 1206 Resistor\nR2 - 1.8k Ohm 1206 Resistor\nR3 - 10k Ohm 1206 Resistor\nR4 - 150 Ohm 1206 Resistor\nR5 - 10k Ohm 1206 Resistor\nS0 - PTS645SM43SMTR92LFS - Surface Mount 6mm Tact Switch\nS1 - 3FSH9 - Surface Mount Tact Switch\nS2 - 3FSH9 - Surface Mount Tact Switch\nS3 - 3FSH9 - Surface Mount Tact Switch\nS4 - 3FSH9 - Surface Mount Tact Switch\nS5 - 5GSH93561 - Surface Mount Tact Switch w/ LED\nT1 - 44121 - 6V Transformer\nU1 - ATMEGA328PB - Microcontroller\nU2 - NCP1117 - 5V 1A LDO Linear Regulator\nY1 - FSRLF - 32.768kHz Crystal
$Comp
L Relay:FINDER-32.21-x000 K1
U 1 1 5F0C6F1B
P 2800 1400
F 0 "K1" H 3230 1446 50  0000 L CNN
F 1 "Relay" H 3230 1355 50  0000 L CNN
F 2 "Relay_THT:Relay_SPDT_Finder_36.11" H 4070 1370 50  0001 C CNN
F 3 "https://gfinder.findernet.com/assets/Series/355/S32EN.pdf" H 2800 1400 50  0001 C CNN
	1    2800 1400
	1    0    0    -1  
$EndComp
$Comp
L Switch:SW_MEC_5G_LED S5
U 1 1 5F144751
P 2350 7050
F 0 "S5" H 2450 7250 50  0000 C CNN
F 1 "GRIND" H 2350 6950 50  0000 C CNN
F 2 "grinder_timer:tact_led" H 2350 7350 50  0001 C CNN
F 3 "http://www.apem.com/int/index.php?controller=attachment&id_attachment=488" H 2350 7350 50  0001 C CNN
	1    2350 7050
	1    0    0    -1  
$EndComp
$EndSCHEMATC
